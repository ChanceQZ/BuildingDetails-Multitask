...
class MultipleConvFCBBoxHead(BBoxHead):
    ...
    def __init__(self,
        ...
        # add fc heads
        self.fc_branch_reg = self._add_fc_branch()
    ...
    def forward(self, x_cls, x_cls_aux, x_reg):
        ...
        # fc head
        x_fc_reg = x_reg.view(x_reg.size(0), -1)
        x_fc = x_cls.view(x_cls.size(0), -1)
        x_fc_aux = x_cls_aux.view(x_cls_aux.size(0), -1)

        for fc in self.fc_branch_reg:
            x_fc_reg = self.relu(fc(x_fc_reg))
        for fc in self.fc_branch:
            x_fc = self.relu(fc(x_fc))
        for fc in self.fc_branch_aux:
            x_fc_aux = self.relu(fc(x_fc_aux))

        bbox_pred = self.fc_reg(x_fc_reg)
