...
def merge_aug_bboxes_multitask(aug_bboxes, aug_scores, aug_scores_aux, img_metas, rcnn_test_cfg):
    ...
    if aug_scores is None or aug_scores_aux is None:
        return bboxes
    else:
        scores = torch.stack(aug_scores).mean(dim=0)
        scores_aux = torch.stack(aug_scores_aux).mean(dim=0)
